...
def modify_feature_scales(batches, arguments):
    log = logging.getLogger("hash")
    extractors = dict((ex.NAME, (ex.NAMESPACE, getattr(arguments, "%s_scale" % ex.NAME)))
                      for ex in __extractors__.values() if "%s_scale" % ex.NAME in dir(arguments))

    for ext in list(extractors):
        if extractors[ext][1] == 1:
            del extractors[ext]

    if not extractors:
        return batches

    err = "You should specify location of docfreq file to modify weights of features"
    ...
    for batch in batches:
        csc_mat = batch.matrix.tocsc()

        for ext in extractors:
            scale = extractors[ext][1]
            ind = indices[ext]
            csc_mat[:, ind] *= scale

        # hack to modify attribute in namedtuple
        batch.matrix.data = csc_mat.tocsr().data
...
def hash_batches(args):
    ...
    batches = modify_feature_scales(batches, args)
